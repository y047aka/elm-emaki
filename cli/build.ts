...
main =
  ...
  await Promise.all([
    // 存在すれは上書きしない
    fs
      .writeFile(path.resolve(projectRootDir, ".gitignore"), gitignore, opt)
      .catch(() => {}),
    fs
      .writeFile(path.resolve(projectRootDir, "elm.json"), elmJson, opt)
      .catch(() => {}),
    fs
      .writeFile(path.resolve(projectRootDir, "src", "Main.elm"), elmMain, opt)
      .catch(() => {}),
  ]);
}
